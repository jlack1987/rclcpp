...
TEST_F(TestNode, subnode_params) {
  ...
  EXPECT_TRUE(subnode->has_parameter(name));
  EXPECT_TRUE(subsubnode->has_parameter(name));
  ...
  // Since node share the same NodeParameters object, the top level node and all subnodes get
  // their callbacks hit even though the parameter that changed belonged to the subsubnode
  // also when the the three callbacks are called, the name of the parameter in each one is 
  // "sub_ns/subsub_ns/a"
  EXPECT_EQ(count, 3);
  EXPECT_EQ(subsubnode->get_parameter(name).as_double(), -1);
